...
| Log says *Manifest signature not verified* (event 1041 in the renewal, 1141 in the creator, 1246 in bootstrap) | The scripts verify a signature on the update manifest. Every production release is signed, so this warning should only appear against a test manifest. If you see it against the production URL, tell the cert team and change nothing on the server. Renewal itself continues normally. The same run also reports a `manifest-unverified` line to the monitoring log, so the cert team normally sees it without being told. |
| Log says *manifest signature refused* (event 1040) and the scripts stop updating | The manifest carries a signature that does not verify. The scripts deliberately refuse to update and keep renewing certificates with the version they have. Tell the cert team; do not download scripts by hand. |
